...
// --- 1. IMAGINI: FOLOSIM UNSPLASH/POLLINATIONS (GRATIS & STABIL) ---
export async function generateImageForPost(postText: string, brandProfile?: BrandProfile): Promise<string> {
  ...
  // Folosim Pollinations AI (gratuit, fără API key, rapid)
  return `https://image.pollinations.ai/prompt/${cleanPrompt}?width=1080&height=1080&nologo=true&seed=${Math.floor(Math.random() * 1000)}`;
}
...
export async function generateImageVariation(base64ImageData: string, mimeType: string, style: string): Promise<string> {
  ...
  return `https://image.pollinations.ai/prompt/${cleanStyle}?width=1080&height=1080&nologo=true&seed=${Math.floor(Math.random() * 1000)}`;
}

// --- 2. LOGO OVERLAY (Funcția care lipsea și dădea eroare la Build) ---
...
export async function overlayLogoOnImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    mainImg.onload = () => {
      ...
      logoImg.onload = () => {
        if (ctx) {
          const logoWidth = canvas.width * 0.20; // Logo = 20% din lățime
...
// --- 3. TEXT: RĂMÂNE PE GOOGLE GEMINI ---
