...
const SocialSparkApp: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // --- AICI A FOST PROBLEMA: DETERMINARE TIP ---
      ...
      const newPostsData = generatedPosts.map(p => ({ 
          ...
          isLocked: false,
          generationType: genType, // Setăm tipul corect
          type: p.type || 'post'
      }));
      ...
      // --- AUTO-SAVE (CRITIC) ---
      if (user) {
          console.log(`💾 Saving ${newPostsData.length} posts to Vault...`);
          ...
          for (const postData of newPostsData) {
              try {
                  ...
                  if (savedId) {
                      ...
                      // Update UI cu ID-ul real din DB
