...
const SocialSparkApp: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // --- AUTO-SAVE ROBUST (Aici este modificarea cerută) ---
      if (user) {
          const limit = userProfile?.subscriptionTier === 'agency' ? 50 : 20;
          console.log(`💾 Auto-Saving ${newPostsData.length} posts...`);

          Promise.all(newPostsData.map(async (postData) => {
              // Nu salvăm erorile
              if (typeof postData.content === 'string' && postData.content.startsWith('⚠️ Error')) return null;

              try {
                  const savedId = await savePostToHistory(user.uid, postData, topic, limit);
                  return { oldId: postData.id, newId: savedId };
              } catch (e) {
                  console.error("Failed to save single post:", e);
                  return null;
              }
          })).then((results) => {
              // După ce s-au salvat, actualizăm ID-urile în UI
              setPosts(current => {
                  const map = new Map();
                  results.forEach(r => { if(r && r.newId) map.set(r.oldId, r.newId); });
                  
                  return current.map(p => map.has(p.id) ? { ...p, id: map.get(p.id)! } : p);
              });
              console.log("✅ Sync complete.");
          });
      } else {
          console.warn("User not detected for Vault Save");
      }
