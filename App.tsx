...
const SocialSparkApp: React.FC = () => {
  ...
  const handleGenerate = async () => {
    if (!topic.trim() && !attachedImage) { setError("Please add content first."); return; }
    ...
    try {
      ...
      // Determinăm tipul pentru salvare corectă
      ...
      const newPostsData = generatedPosts.map(p => ({ 
          ...
          isLocked: false,
          generationType: genType, // IMPORTANT: Salvăm tipul corect
          type: p.type || 'post'
      }));
      ...
      if (user) {
          ...
          // Salvăm fiecare post și actualizăm ID-ul local cu cel din DB
