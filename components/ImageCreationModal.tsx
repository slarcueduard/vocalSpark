...
export function ImageCreationModal({ onClose, onSelectImage, initialPrompt = '' }: ImageCreationModalProps) {
  ...
  const handleGenerate = async () => {
    ...
    if (!checkCredits(currentCost)) { setError(`Not enough credits.`); return; }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    // Creăm Blob URL instant (nu ocupă memorie JS heap)
  ...
  // --- LOGICĂ OPTIMIZATĂ: FĂRĂ CANVAS DACA NU E NEVOIE ---
  const handleUseImage = async () => {
      const target = activeTab === 'upload' ? uploadedImage : resultImage;
      if (!target) return;

      // 1. FAST PATH: Dacă nu avem filtru selectat, trimitem URL-ul direct.
      // Asta rezolvă crash-ul pe mobil pentru 90% din cazuri.
      if (selectedPhotoFilter === 'normal') {
          onSelectImage(target);
          onClose();
          return;
      }

      // 2. SLOW PATH: Doar dacă userul vrea neapărat filtru ars pe imagine
      ...
      try {
          ...
          console.error(e);
          // Fallback la imaginea originală dacă crapă procesarea
          onSelectImage(target);
          onClose();
      } finally {
  ...
  const previewSrc = activeTab === 'generate' ? resultImage : uploadedImage;
      ...
      <div className="w-full max-w-5xl bg-[#0f1115] border border-gray-800 rounded-2xl overflow-hidden flex flex-col md:flex-row h-[85vh] shadow-2xl">
        ...
        <div className="w-full md:w-1/2 p-6 flex flex-col bg-[#161b22] border-r border-gray-800 overflow-y-auto">
             <div className="flex justify-between items-center mb-6">
                ...
                <div className="bg-gray-800 px-3 py-1 rounded-full border border-gray-700 text-xs text-white font-bold">Credits: {credits}</div>
             </div>
             ...
             {(activeTab === 'upload' && uploadedImage) && (
                 <div className="mt-4">
                     <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Filters</label>
                     <div className="grid grid-cols-4 gap-2">
                         {PHOTO_FILTERS.map(f => (
                             <button key={f.id} onClick={() => setSelectedPhotoFilter(f.id)} className={`p-2 rounded-lg border text-center text-[10px] font-bold ${selectedPhotoFilter === f.id ? 'border-blue-500 text-white' : 'border-gray-700 text-gray-400'}`}>
                                 {f.label}
                             </button>
                         ))}
                     </div>
                     <button onClick={() => { setUploadedImage(null); fileInputRef.current!.value=''; }} className="mt-4 w-full py-2 border border-red-900/50 text-red-400 text-xs rounded-lg">Remove Image</button>
                 </div>
             )}
            ...
            {previewSrc ? (
                <div className="flex flex-col items-center w-full gap-4">
                    <img 
                        ...
                        className="max-h-[400px] max-w-full rounded-lg shadow-2xl object-contain" 
                    />
