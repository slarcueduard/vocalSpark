...
import { 
    ...
    Image as ImageIcon, Share2, Smartphone, Lock, Unlock
} from 'lucide-react';
...
export const PostCard: React.FC<PostCardProps> = ({ 
  ...
  const [shareMessage, setShareMessage] = useState(''); // Mesaj vizual pt user
  ...
  const handleNativeShare = async () => {
    ...
    setIsSharing(true);
    setShareMessage("Copying Caption..."); // Feedback vizual

    try {
        // 1. COPIEM TEXTUL ÎN CLIPBOARD (Best practice pt FB/Insta)
        await navigator.clipboard.writeText(contentToShare);
        setShareMessage("Caption Copied! Opening Share...");
        
        // Mică pauză ca userul să vadă mesajul
        await new Promise(r => setTimeout(r, 800));

        // 2. PREGĂTIM DATELE DE SHARE
        const shareData: any = {
            title: 'New Social Post',
            // Trimitem textul și aici, dar majoritatea apps îl ignoră dacă e fișier
            text: contentToShare 
        };

        // 3. PREGĂTIM IMAGINEA (Dacă există)
        if (post.imageUrl) {
             // Dacă e URL extern (Pollinations) sau Base64, trebuie făcut Blob
        ...
        // 4. LANȘĂM SHARE SHEET
        await navigator.share(shareData);
        setShareMessage(""); // Reset la final
  ...
  const platforms = [
      ...
      { id: Platform.TikTok, icon: Video, color: 'hover:text-cyan-400' },
  ...
  return (
    <div className="bg-[#161b22] border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-600 transition shadow-xl flex flex-col md:flex-row group relative">
        
        {/* Mesaj Overlay pentru Share */}
        {shareMessage && (
        ...
        <div className="flex-1 p-6 flex flex-col">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-1">
                    <button onClick={() => onToggleLock(post.id)} className="p-2 text-gray-600 hover:text-yellow-500 transition">{post.isLocked ? <Lock size={16} /> : <Unlock size={16} />}</button>
            ...
            <div className="flex-1 mb-6 relative min-h-[120px]">
                ...
                <textarea readOnly className="w-full h-full bg-transparent text-gray-300 text-sm leading-relaxed resize-none outline-none cursor-text scrollbar-hide" value={displayContent} />
            </div>
            ...
            <div className="border-t border-gray-800 pt-4 space-y-5">
                <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                    {platforms.map((p) => {
                        ...
                        return (
                            <button key={p.id} onClick={() => { setActiveTab(p.id); if (!hasContent) onAdaptPost(post.id, p.id, post.content); }} className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all duration-200 ${isActive ? 'bg-blue-600 text-white border-blue-500 shadow-lg' : hasContent ? 'bg-gray-800 text-gray-300 border-gray-500' : 'bg-transparent text-gray-700 border-gray-800'}`}>
                                <Icon size={16} />
                            </button>
                        );
                    })}
