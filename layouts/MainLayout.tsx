        ...
        <div className="p-4 border-t border-gray-800 bg-[#161b22]">
          <div className="bg-[#0f1115] rounded-xl p-4 border border-gray-800">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded text-black ${isPremium ? 'bg-purple-400' : 'bg-yellow-500'}`}>{planName}</span>
            </div>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center overflow-hidden text-sm font-bold shrink-0">{user?.email?.[0].toUpperCase()}</div>
              <div className="overflow-hidden">
                <p className="text-sm font-medium truncate w-32">{user?.email?.split('@')[0]}</p>
                <button onClick={() => logout()} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 mt-0.5"><LogOut size={10} /> Sign Out</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#0f1115] relative transition-all duration-300">
        ...
        <header className="h-16 border-b border-gray-800 bg-[#0f1115]/80 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <span className="px-2 py-1 rounded border border-blue-500/30 bg-blue-500/10 text-blue-400 text-xs font-mono hidden sm:block">v1.5 PRO</span>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            >
              ...
              <span className="text-sm font-medium text-gray-200">
                ...
                <span className="text-white font-bold group-hover:text-yellow-400 transition">{credits}</span>
              </span>
            </div>
